...
test<-Default%>% 
  ...
  select("default","student","balance","income")   ###================basecase1==============
...
training_model<-rpart(default~student+balance+income,
                      data=training, #what is used for classification model
                      method="class", 
                      control=rpart.control(cp=0.03))   ########======================p=.8 check in basecase1
...
```{r}
test$ct_pred_prob<-predict(training_model,test)[,2]  #tree proabtilitystores the predicted probability of default as new variable
head(test)
test$ct_pred_class<-predict(training_model,test,type="class") ####binomial yes or no
test
```

Check the accuracy of the model by comparing the the actual values (default) and the predicted values (ct_pred_class).
```{r}
table(test$default==test$ct_pred_class)  ##compare test variable probability with predicted class 
1937/2000
##run and could get manually
sum(predict(training_model,test,type="class")==test$default)/nrow(test) #is number of correctly predicted cases/total number of cases in       TEST       data set value the sames as the actual 

sum(predict(training_model, training,type="class")==training$default)/nrow(training)

#THIS IS HOLDOUT VALIDATION - total number of training datat set demiminator is number of correctly predecit in TRAINING, HIGH BECAUSE MODEL IS BUILT ON TRAINING DATA

```

You may also want to check a confusion table. 
```{r}
table(test$ct_pred_class,test$default, dnn=c("predicted","actual"))  # confusion table on test data, THE ACTUAL CLASS IS NO AND YES AND THE TOTAL NUMBER OF YES IN TEST DATA IS 70 WHERE 21 WERE PREDICTED AS 
```
...
full_tree<-rpart(default~student+balance+income,
                     data=training, 
                     method="class",
                     control=rpart.control(cp=0)) #CLOSER TO ZERO, COMPLEXITY INCREASES - ALLOWING SPLITS, TRESS TO GROW THIS ALLOWS PURE (AND OVERFITTING) CONTINUE TO GO BASECASE1
...
```{r}
printcp(full_tree)   # xerror, xstd - cross validation results  ===============BASECASE1==========SLIGHTLY HIGHER ERROR XERROR BECAUSE IT'S USING A FOLD THAT WAS LEFT OUT - SO THE FIRST SPLIT MIGHT NOT GENERALIZES AS WELL, ASFTER TO THE % OF ROOT NODE ERROR SHOULD GO DOWN = READ THIS 1,2,3,4, -  THIS IS THE SAME AS EPOC TESTING PREFORMANCE IN NN  THEN IN WHAT 


## IDEAL TREE SHAPE HERE BY IDENTIFYING # OF SPLITS WITH LOWEST XSTD WHERE LOWEST AND NOT INCREASING 
...
```{r}
plotcp(full_tree)    ###THE BASECASE CHOICE IT MIGHT BE RELEVANT TO CHOOSE LESS SPLITS WITHIN RELATIVE SD RANG 
```
...
# prune tree with minimum cp value  BASECASE CHOICE OF MIN 
...
```{r}
bp_tree<-min_xerror_tree  #HYPER TUNED TO LOWEST CROSS VALIDATION ERROR SO APPLIED TO TEST DATASET TO GET PREDICTED PROBABILITY OF THE DEFAULT AND PREDICTED CLASS OF DEFAULT ===========BASECASE==============THIS IS BASICALLY ANOTHER ITERATION OF THE ABOVE BUT WITH THE BEST FITTING MODEL=
test$ct_bp_pred_prob<-predict(bp_tree,test)[,2]
test$ct_bp_pred_class=ifelse(test$ct_bp_pred_prob>0.5,"Yes","No")
head(test)

table(test$ct_bp_pred_class!=test$default)  # error rate NUMBER OF INCORRECTLY PREDICTED CASES basecase

63/2000 # error rate
1937/2000 #accuracy measure

```
```{r}
table(test$ct_bp_pred_class,test$default, dnn=c("predicted","actual"))  # confusion table on test data
```
## Random Forest - the concept conducts holdout validatoin because randomforest is bootstrapped anyway without replacement
...
```{r}
# Execute the tuning process #y variable predictive target and starting to try two variables then review error rate basecase out of bag error rate, need to provide the mtry number lowest number of variables and error rate - random forest is going to build many class tree and in each split of each tree it will consider the randomly choosen variable an dprovides einformation gain after error, so it's randomly out of bahg of class of tree of random splits
set.seed(1)              
...
rf_best_model<-randomForest(default~income+balance+student,              # model formula
                       ...
                       mtry=1,
                       importance=TRUE)
rf_best_model #so of these trees are we improving the above 
...
glimpse(test)
#BASE CASE UPDATE ERROR RATE  AND ACCURACY
60/2000 #ERROR
1940/2000 #ACCURACY
...
svm_tune <- tune(svm,                            # find a best set of parameters for the svm model      
                 ...
                 data = training, #USED TO BUILD MODEL
                 kernel="radial", #IS NON-LINEAR MODEL B/C PREDICTS ALL WILL NOT DEFAULT
                 ranges = list(cost = 10^(-5:0))) # specifying the ranges of parameters  COST (OF VARIABLES)
                                                  # in the penalty function to be examined
                                                  # you may wish to increase the search space like 
                                                  

print(svm_tune)                              # best parameters for the model  .02775 IS ERROR RATE, WHEN COST IS 1
best_svm_mod <- svm_tune$best.model    #AUTO SAVED BEST MODEL 

hist(best_svm_mod$decision.values) #SUM(BEST_SVM_MOD$DECISION.VALUES <0)
...
60/2000  #maybe higher lower than training impliing it's better than a eal world process
```
...
sum(test$logit_pred_class!=test$default)/nrow(test)  #figure out why ! fixes the ordering 

```

### step-wise regression
to improved the LOGIT regression preforamnce ***** it's important to underrstand which variables should be included *** setp-wise regression help variable selection - assess model preformance by adding variable and compate forward and backwards, remember specific null model and full model then run forward model null is gradually adding variable, low the scoe is forwrad - higher it's backwardd 

did this choose the same variables?? 

redo the model using these variables 
```{r}
...
sum(test$logit_pred_prob!=test$default)/nrow(test)

sum(test$logit_pred_class!=test$default)/nrow(test)    ### this is your final answer for hte logistic regression fine tuned by stepwise function hyper paramater model tested best fit model error rate - accuracy is then againt the test data and error rate is now 2.85% and accuracy is 97.15%
